...
| Downloads | Dedicated authenticated download centre for a tested unsigned Apple-silicon macOS pilot `.dmg` and the separate macOS/Windows Python 3 cloud connection utility. Platform requirements, secure pairing route and four-step setup guide are explicit. Signed/notarised customer installers remain pending release approval. |
...
Cloud evidence integration candidate - 14 September 2026: the existing laptop companion now creates one bounded JPEG overview for eligible observations, persists an encrypted local media outbox, retries after offline periods and delivers a manifest followed by content to the cloud. The cloud stores encrypted evidence with branch-inclusive foreign keys, exposes only nested branch-authorised alert retrieval, reports `NONE`, `PENDING`, `PARTIAL`, `READY` and `EXPIRED` aggregate states, and runs bounded expiry cleanup. The management console renders `OVERVIEW`, `INTERACTION_CROP` and `CLIP` evidence using the same contract and preserves review when media is absent, pending, expired, deleted or unavailable. Local verification passed 752 API tests with one skip, 459 cloud/PostgreSQL tests with 22 skips, 21 control unit tests and all 10 control browser journeys. The cloud evidence mode still defaults to `METADATA_ONLY`; real Render evidence storage configuration, customer-device credentials, packaged Windows/macOS runtime checks and authorised pharmacy footage remain external acceptance work.

Private R2 storage candidate - 14 September 2026: the cloud evidence boundary now includes a create-only Cloudflare R2 S3 backend restricted by configuration to the EU jurisdiction, rejects filesystem evidence on Render, keeps application AES-256-GCM envelope encryption, and reauthorizes after every blob operation without holding PostgreSQL transactions across network I/O. Startup now verifies conditional create, bounded read, delete and absence with a private random canary; deletion failures use bounded backoff so a bad old object does not starve newer cleanup; and a strictly validated versioned KEK keyring keeps retained evidence readable across rotations, including manifests uploaded after the current write version changes. The full cloud/PostgreSQL suite passed 475 tests with 22 skips, including synthetic provider, strict missing-object classification, concurrency, revalidation, cleanup-fairness and old/new key coexistence checks. No Cloudflare bucket or credential has been supplied, no real R2 round trip or Render restart-persistence check has run, and Render remains `METADATA_ONLY`. Follow [the R2 release gates](docs/release/cloud-evidence-r2.md) before activation.
