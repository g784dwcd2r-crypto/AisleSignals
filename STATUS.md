...
Private R2 storage candidate - 14 September 2026: the cloud evidence boundary now includes a create-only Cloudflare R2 S3 backend restricted by configuration to the EU jurisdiction, rejects filesystem evidence on Render, keeps application AES-256-GCM envelope encryption, and reauthorizes after every blob operation without holding PostgreSQL transactions across network I/O. Synthetic unit and PostgreSQL race/failure checks cover conditional writes, bounded reads, provider failures, concurrent retry, withdrawal/expiry revalidation and cleanup. No Cloudflare bucket or credential has been supplied, no real R2 round trip or Render restart-persistence check has run, and Render remains `METADATA_ONLY`. Follow [the R2 release gates](docs/release/cloud-evidence-r2.md) before activation.
