...
The approved central operations programme is [Plan B](docs/release/plan-b.md): a role-scoped multi-pharmacy dashboard, freshness-aware four/six-camera branch views, privacy-first metadata alerts, optional encrypted snapshots/clips under controller-approved evidence mode, temporary audited live access, central review and bounded remote diagnostics. Plan B is planned work; it is not implemented or commissioned yet.
