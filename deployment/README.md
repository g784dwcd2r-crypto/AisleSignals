...
The macOS CI job also wraps the verified Apple-silicon pilot bundle as `AisleSignals Pilot.app` inside an unsigned `.dmg`. Drag the application into Applications and open it only when authorised by the pharmacy's installation policy. The pilot disk image is not Apple-signed or notarised, does not include the optional 3.3 GB interaction model, and does not replace the separate cloud connection companion. Production distribution still requires Developer ID signing, notarisation, update lifecycle checks and acceptance on the exact pharmacy Mac.
