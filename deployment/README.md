...
Ctrl+C gracefully stops only the launcher's children, escalating to terminate/kill if necessary, and does not delete the database or model weights. If an owned process exits, recovery is bounded by `restart_limit`; a permanently failed model leaves casework degraded, while a permanently failed API shuts down the launch. If another process takes a released port, it is not adopted or killed.
