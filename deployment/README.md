...
The macOS CI job also wraps the verified Apple-silicon pilot bundle as `AisleSignals Pilot.app` inside an unsigned `.dmg`. Drag the application into Applications and open it only when authorised by the pharmacy's installation policy. A normal Finder launch starts in casework-only mode so the core pilot works before the optional model is installed; command-line arguments continue to select explicit advanced launch modes. The pilot disk image is not Apple-signed or notarised, does not include the optional 3.3 GB interaction model, and does not replace the separate cloud connection companion. Production distribution still requires Developer ID signing, notarisation, update lifecycle checks and acceptance on the exact pharmacy Mac.
...
Open the loopback URL printed by the launcher, sign in with a named account and explicitly select the authorised CCTV window/camera or test recording. Test view visibility, source freshness and the existing speakers with an on-site staff member. Keep the launcher window open. API/model health alone never proves that a camera is monitored or that an alarm can be heard.
