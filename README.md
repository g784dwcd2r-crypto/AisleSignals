...
Planned stack: React/TypeScript, FastAPI, PostgreSQL, durable background workers, private evidence storage and a local gateway when required. The detector is a replaceable integration; a generic object model is not a validated theft detector.
...
Live operation requires site-specific processing records, exact camera and supplier access, trained reviewers and actual acceptance evidence. Hardware ownership/funding, detector fees, tax treatment and human operational coverage remain discovery inputs. Subscription records begin as owner-managed billing; this repository does not charge customers.
