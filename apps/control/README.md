...
- The Downloads tab provides the tested unsigned Apple-silicon macOS pilot `.dmg` and Windows x64 pilot `.exe` from the exact checked prerelease, plus the separate Python 3 connection utility, platform-specific requirements and a direct route into secure pairing. The page distinguishes the local pilot application from cloud connection and states that signed/notarised customer installers remain pending.
