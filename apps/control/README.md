...
- The Downloads tab provides the current Python 3 connection utility for macOS and Windows, platform-specific requirements and a direct route into secure pairing. The page explicitly identifies the two-file ZIP as a connection utility rather than an installer, and states that the macOS `.dmg` and Windows `.msi` are unreleased until their release gates pass.
