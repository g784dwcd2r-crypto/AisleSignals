...
def test_actual_missing_schema_then_idempotent_migration_and_readiness(empty_database):
    ...
    with psycopg.connect(empty_database.database_url) as connection:
        ...
        assert {row[0] for row in tables} == {"schema_version", "organisations", "pharmacies", "users", "user_pharmacies", "sessions", "invitations", "auth_challenges", "auth_attempts", "audit_entries", "device_enrolments", "devices", "alerts", "incidents", "device_sync_receipts", "evidence_objects"}
