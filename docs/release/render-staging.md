...
An optional attended Python 3 helper is downloadable from the Laptops view. It currently supports private credential storage on macOS/Linux, reports connection presence only and sends monitoring UNKNOWN with zero cameras. It does not capture CCTV, send alerts, start at login or run as a managed background service. Its credential operations fail closed on Windows pending independently verified native ACL/reparse protection; Windows browsers and the separate Windows pilot are unaffected. Integrating the existing detection application's durable outbox and packaging secure Windows cloud pairing remain separate work.
