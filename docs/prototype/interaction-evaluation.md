...
- Simulated false-alarm episodes: alarm-eligible results on qualifying normal sessions must pass a 15-second measured evaluation processing-time proxy for the browser freshness gate, then are grouped using the configured source-time cooldown (default 30 seconds). An eligible prediction without `wall_ms` makes that normal session ineligible for an alarm-rate claim. The replay does not reproduce live queue and polling delay, live processing gaps, real alert queueing or physical sound. Without qualifying normal duration the rate is `null`.
