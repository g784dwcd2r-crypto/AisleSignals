...
The optional xAI second-stage reviewer has separate `GET/PUT /api/xai-verifier` branch configuration, `GET /api/xai-verifications/{id}` scoped records and `POST /api/interactions/{id}/xai-verification`. It is disabled by default and currently restricted to manager-approved public staged recorded-video trials; live/private pharmacy frames fail closed because a commissioned pixel mask is not implemented. A local candidate gates access, but xAI blindly classifies the complete three-to-six-frame sequence without receiving the local action. Deterministic code compares results afterward. Records are bounded, expire with the source and cannot actuate or make an alarm eligible. Feature flags are branch-scoped; one installation-global pilot ledger reserves USD 1 per call and has a hard USD 5 ceiling. See [xai-incident-verifier.md](xai-incident-verifier.md).
...
The **Video test** page supports user-selected MP4/WebM playback, deterministic frame-change analysis and optional local JSON summary. An explicitly started **alarm playback test** adds local attention tones and persistent branch-scoped event metadata through `GET/POST /api/playback-events`; see [playback API](playback-alerts-api.md) and [alarm guide](attention-alarm-test.md). File content and filenames never reach the server. The CSP allows local blob media and a click-loaded YouTube privacy-enhanced reference iframe. That external player is not analysed. See [video-test.md](video-test.md) for limits, lifecycle and accuracy boundaries.
